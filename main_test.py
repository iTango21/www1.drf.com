...
output_json_file_path = "main_test.json"
output_mod_file_path = "main_test_mod.json"
    ...
    if isinstance(data, list):
        ...
        for key in list(data.keys()):
            if key in keys_to_remove:
                ...
                recursive_remove_keys(data[key])
...
# Write changed data to JSON-file
def write_output_mod_data(data_mod):
    # Removing extra spaces
    remove_extra_spaces(data_mod)
    with open(output_mod_file_path, "w", encoding='utf-8') as file:
        json.dump(data_mod, file, indent=4, ensure_ascii=False)


def read_input_data(date):
    cookies = {
    ...
    response = requests.get(
        f'https://www.drf.com/results/resultDetails/id/PRM/country/USA/date/{date}',
        cookies=cookies,
        headers=headers,
    )
...
if __name__ == "__main__":
    ...
    input_data = read_input_data(date_)
